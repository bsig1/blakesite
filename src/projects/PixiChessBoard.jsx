...
const PixiChessBoard = forwardRef(function PixiChessBoard(
  ...
  }, [size, colors, squareSize, onSquareClick]);
    ...
    (async () => {
      ...
      for (let i = 0; i < 64; i++) {
        ...
        spr.on("pointerdown", () => {
            ...
            selected.current = i;
            selected_square.current = highlightSquare(i);
        });
  ...
  const drawLastMove = (from, to) => {
    ...
    [from, to].forEach((sq) => {
      const fx = fileToX(sq[0]);
      const ry = rankToY(parseInt(sq[1], 10));
      g.beginFill(colors.lastMove);
      g.drawRect(fx * squareSize, ry * squareSize, squareSize, squareSize);
      g.endFill();
    });
    ...
    function highlightSquare(index, color = 0xF7EC6C) {
        ...
        g.drawRect(fileIdx * s, rankIdx * s, s, s);
        g.endFill();
